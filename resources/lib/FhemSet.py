...
##########################################################################################################################
    def SendCommand(self,ip,port,command):
        ...
        PORT=port
        CMD=command
        print "[plugin.program.fhemcontrol] ip=%s" % (ip)
        print "[plugin.program.fhemcontrol] port=%s" % (port)
        print "[plugin.program.fhemcontrol] cmd=%s" % (command)
        ret = ""
        try:
          tn = telnetlib.Telnet()
...
##
##########################################################################################################################
    def GetState(self,ip,port,command):
        WINDOW = xbmcgui.Window( 10000 )
        IP=ip
        PORT=port
        CMD=command
